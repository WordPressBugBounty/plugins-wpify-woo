...
class Get_Command extends \ActionScheduler_WPCLI_Command
{
    ...
    public function execute()
    {
        ...
        if (is_a($action, \Action_Scheduler\WP_CLI\Action\ActionScheduler_NullAction::class)) {
            ...
            \WpifyWooDeps\WP_CLI::error(sprintf(esc_html__('Unable to retrieve action %d.', 'action-scheduler'), $action_id));
        }
        ...
        if ($only_logs) {
            $args = array('format' => \WpifyWooDeps\WP_CLI\Utils\get_flag_value($this->assoc_args, 'format', 'table'));
            $formatter = new \WpifyWooDeps\WP_CLI\Formatter($args, array('date', 'message'));
